...
{
    public class DictionaryDuplicateHandling
    {
        public static void excelToDictionaryDuplicate()
        {
            ...
            using (OleDbConnection conn = new OleDbConnection(connectionString))
            {
                ...
                while (reader.Read())
                {
                    ...
                    {
                        Console.WriteLine($"Duplicate ID found: {id}. Keeping first entry ({employee[id].name}, {employee[id].age}), ignoring new ({name}, {age}).");
                        continue;
                    }
